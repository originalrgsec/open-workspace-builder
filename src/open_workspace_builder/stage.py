"""Stage assessment and promotion logic (PRD stages 0-1)."""
...
class StageEvaluator:
    ...
    def assess(self, target_stage: int) -> StageAssessment:
        ...
        if target_stage < current:
            raise ValueError(f"Cannot promote backwards from stage {current} to {target_stage}")
    ...
    def _check_exit_criteria(self, current: int, target: int) -> tuple[CriterionResult, ...]:
    ...
    def _check_structural_files(self) -> CriterionResult:
        ...
        missing = [f for f in _REQUIRED_STRUCTURAL_FILES if not (self._vault_path / f).is_file()]
        if missing:
    ...
    def _check_project_scaffolded(self) -> CriterionResult:
        ...
        has_project = any(f.is_file() for f in projects_dir.rglob("status.md"))
        if not has_project:
            return CriterionResult(
                name="Project scaffolded",
                passed=False,
