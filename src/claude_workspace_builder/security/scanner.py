...
class Scanner:
    ...
    def _get_patterns(self) -> list:
        ...
        if self._loaded_patterns is None:
            from claude_workspace_builder.security.patterns import load_patterns
    ...
    def scan_file(self, path: Path) -> ScanVerdict:
        ...
        if 1 in self._layers:
            from claude_workspace_builder.security.structural import check_structural

            all_flags.extend(check_structural(path))

        if 2 in self._layers:
            from claude_workspace_builder.security.patterns import check_patterns

            all_flags.extend(check_patterns(path, self._get_patterns()))

        if 3 in self._layers and self._api_key:
            from claude_workspace_builder.security.semantic import analyze_content
