...
class TestMigrateSecurityBlocking:
    ...
    def test_flagged_file_is_blocked(
        ...
        with patch(
            "claude_workspace_builder.engine.migrator.Scanner"
        ) as MockScanner:
...
class TestMigrateDryRun:
    ...
    def test_dry_run_no_files_written(
        ...
        files_after = set()
        for f in drifted_workspace.rglob("*"):
            if f.is_file() and ".cwb" not in str(f.relative_to(drifted_workspace)):
                files_after.add((str(f.relative_to(drifted_workspace)), f.read_bytes()))

        # The non-.cwb files should be identical
    ...
    def test_dry_run_no_migration_log(
        ...
        log_file = drifted_workspace / ".cwb" / "migration-log.jsonl"
