...
```bash
owb migrate ./output           # interactive review of each change
owb migrate ./output --accept-all  # batch mode, skip prompts
...
`owb` includes a three-layer security scanner designed to catch malicious content in workspace configuration files — particularly important when pulling content from external sources like ECC.
...
The scanner runs automatically during `owb migrate` and `owb ecc update`. Files flagged as malicious are blocked from being applied. A reputation ledger tracks flag events per source; if a source exceeds the malicious flag threshold, owb recommends dropping that upstream entirely.

Layer 3 (semantic analysis) requires the `security` extra and an `ANTHROPIC_API_KEY`:

```bash
pip install "open-workspace-builder[security] @ git+https://github.com/VolcanixLLC/open-workspace-builder.git"
...
`owb` works without any configuration file — sensible defaults are built in. To customize, create a `config.yaml` and pass it with `--config`:

```bash
owb init --config config.yaml
```
...
```bash
owb ecc update          # interactive review with security scanning
owb ecc status          # check current pinned commit
```
...
Add the skill name to `config.yaml` under `skills.install` and run `owb init`.

## Requirements

- Python 3.10+
- Core: `click` (installed automatically)
- Optional: `pyyaml` for custom config files (`pip install "open-workspace-builder[yaml] @ git+https://github.com/VolcanixLLC/open-workspace-builder.git"`)
- Optional: `anthropic` for Layer 3 semantic scanning (`pip install "open-workspace-builder[security] @ git+https://github.com/VolcanixLLC/open-workspace-builder.git"`)
